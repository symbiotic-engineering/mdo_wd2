...
def desalandpto_objective(ind):
    Runner = RunWDDS(eng)
    out = Runner.desalandpto(ind, HYDRO_RESULTS)
    return out['LCOW']
...
DESALANDPTO_VARS = ["hinge2joint", "piston_area", "accum_volume", "accum_P0", "capacity"]
DESALANDPTO_BOUNDS = {var: BOUNDS[var] for var in DESALANDPTO_VARS}
DESALANDPTO_BITS = {var: BITS[var] for var in DESALANDPTO_VARS}
print("Starting optimization...")
desalandpto_ga = GA(safe_desalandpto_objective, DESALANDPTO_BOUNDS, DESALANDPTO_BITS,
                    NGEN=800, NPOP=160, NWORKERS=PARAMS["nworkers"],
                    CXPB=0.8, MUTPB=0.03, ELITES_SIZE=2, TOURNAMENT_SIZE=3,
                    PATIENCE=100, TOL=1e-3, csv_path="data/newresults_desalandpto.csv")

results = desalandpto_ga.run()
