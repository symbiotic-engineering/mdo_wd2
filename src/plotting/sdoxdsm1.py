...
x.add_system("lcof", FUNC, r"\text{LCO Flow}")
...
x.connect("opt2", "hydraulics", r"A_p,V_{acc},P_0")
...
x.connect("rigidbody", "lcof", r"\text{stroke}")
x.connect("hydraulics", "lcof", r"Q_p")
x.connect("opt2", "lcof", r"A_p,V_{acc}")
x.connect("lcof", "opt2", r"\text{LCO Flow}")

# Feed optimal results to Stage 3:
x.connect("opt2", "rigidbody2", r"\ell_1^*")
x.connect("opt2", "hydraulics2", r"A_{p,2}^*, V_{acc}^*, P_0^*")
x.connect("opt2", "econ", r"A_{p}^*, V_{acc}^*")

# === Stage 3: Desalination & Economics ===
# Stage 3 optimizer drives desalination and economics
x.connect("opt3", "desal", r"Q_{p,max}")
x.connect("opt3", "econ", r"Q_{p,max}")
x.connect("desal","hydraulics2", r"\begin{array}{c} \Delta \pi, R_m, R_t, \\ P_{relief} \end{array}")
...
x.connect("rigidbody2", "econ", r"\text{stroke}")
x.connect("hydraulics2", "econ", r"Q_f,Q_p")
x.connect("desal", "econ", r"Q_{f,max}")
x.connect("econ", "opt3", r"\text{LCOW}")
...
x.add_input("rigidbody", r"\text{draft}, \ell_2, \ell_3, \mathbf{p}_{\text{waves}}")
x.add_input("rigidbody2", r"\text{draft}, \ell_2, \ell_3, \mathbf{p}_{\text{waves}}")
x.add_input("hydraulics", r"\begin{array}{c} \Delta \pi_0, R_{m,0}, R_{t,0}, \\ P_{relief} = \infty \end{array}")
...
x.add_output("opt2", r"\begin{array}{c} \ell_{1}^*, A_{p,2}^*, V_{acc}^*, P_0^* \\ \text{(stage2 choices)} \end{array}", side=LEFT)
x.add_output("opt3", r"\begin{array}{c} Q_{p,max}^*\\ \text{(stage3 choice)} \end{array}", side=LEFT)
